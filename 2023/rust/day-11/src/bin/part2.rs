...
fn process(input: &str, scale: usize) -> String {
    let map = input
        .lines()
    ...
    let res = galaxies
        ...
        .map(|(row, col)| {
            (
                row + rows.as_slice()[..row].iter().sum::<usize>() * (scale - 1),
                col + cols.as_slice()[..col].iter().sum::<usize>() * (scale - 1),
            )
        })
        .collect::<Vec<_>>()
        .into_iter()
