...
def get_max(curr_valve, rates, costs, to_visit, time):
    ...
    for valve in to_visit:
        ...
        # recurse
        val += get_max(valve, rates, costs, to_visit.difference({valve}), t)
...
def solver(file):
    ...
    max_val = 0
    nodes = set(rates.keys())
    l = len(rates)//4
    for i in range(l, l*3):
        for you_visit in itertools.combinations(nodes, i):
            you_visit = set(you_visit)
            elephant_visit = nodes.difference(you_visit)
            you = get_max('AA', rates, costs, you_visit, 26)
            elephant = get_max('AA', rates, costs, elephant_visit, 26)
            max_val = max(max_val, you + elephant)
    return max_val
